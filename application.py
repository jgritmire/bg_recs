...
@application.route("/gamerecs")

def index():
    flash("Please enter the titles of 3 board games separated by forward slashes (/)")
    return render_template("index.html")

@application.route("/greet", methods=["POST", "GET"])
def greet():
    flash(new_user_recommender(request.form['game_input']))
    return render_template("index.html")
